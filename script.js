...
function addNewExpense() {
  ...
  // Format the amount
  const formattedAmount = '// This separate JavaScript file will handle all dashboard functionality
...
// Function to initialize sidebar navigation - FIXED VERSION
function initializeSidebar() {
  ...
  sidebarLinks.forEach(link => {
    link.addEventListener('click', function(e) {
      // Prevent the default hash link behavior that's causing the issue
      e.preventDefault();
      ...
      // Show the target page - make sure all other pages are hidden
      ...
      console.log('Changed page to:', targetPage);
    });
  });
}
          ...
          <div class="form-row">
            ...
            <div class="form-col">
              <div class="form-group">
                ...
                <select i + parseFloat(expenseAmount).toFixed(2);
...
function openMarkPaidModal(invoiceId, amount) {
  ...
  document.getElementById('paid-amount').value = amount.replace('// This separate JavaScript file will handle all dashboard functionality
document.addEventListener('DOMContentLoaded', function() {
  // Initialize sidebar navigation
  initializeSidebar();
...
function viewInvoice(invoiceId, clientName, issueDate, dueDate, amount, status) {
  ...
  const amountValue = parseFloat(amount.replace('// This separate JavaScript file will handle all dashboard functionality
document.addEventListener('DOMContentLoaded', function() {
  // Initialize sidebar navigation
  initializeSidebar();
...
function createInvoicePreview() {
  ...
  if (!viewInvoiceModal) return false;
  
  // Update invoice header
  viewInvoiceModal.querySelector('.invoice-id').textContent = invoiceNumber;
  viewInvoiceModal.querySelector('.invoice-date:nth-of-type(2)').textContent = `Date: ${moment(issueDate).format('MMM D, YYYY')}`;
  viewInvoiceModal.querySelector('.invoice-date:nth-of-type(3)').textContent = `Due: ${moment(dueDate).format('MMM D, YYYY')}`;
  
  // Update client info
  const billToAddress = viewInvoiceModal.querySelector('.invoice-address:nth-of-type(2) p');
  if (billToAddress) {
  ...
  const invoiceItemsTable = viewInvoiceModal.querySelector('.invoice-items table tbody');
  ...
  const totalsRows = viewInvoiceModal.querySelectorAll('.invoice-total-row');
