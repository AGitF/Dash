...
function updateInvoiceTotals() {
  ...
  if (!subtotalElem || !taxRateElem || !totalElem) return;
  ...
  const taxRate = parseFloat(taxRateElem.value) || 0;
...
function createInvoicePreview() {
  ...
  if (totalsRows.length >= 4) {
    ...
    // Tax
    totalsRows[2].querySelector('.invoice-total-label').textContent = `Tax (${taxRate}%)`;
